...
async function tiktok(sock, chatId, message) {
    try {
        ...
        try {
                ...
                if (apiResponse.data && apiResponse.data.status === 200 && apiResponse.data.tiktok) {
                    ...
                    if (videoUrl) {
                        await sock.sendMessage(chatId, {
                            video: { url: videoUrl },
                            mimetype: "video/mp4",
                            caption: "DOWNLOADED BY EXTREAME-XMD"
                        }, { quoted: message });
                        return;
                    }
                }
            ...
            if (!downloadData || !downloadData.data || downloadData.data.length === 0) {
                return await sock.sendMessage(chatId, { 
                    text: "No media found at the provided link. Please check the link and try again."
                });
            }
            ...
            await sock.sendMessage(chatId, { 
                text: "Failed to download the TikTok video. Please check the link and try again."
            });
        }
    } catch (error) {
